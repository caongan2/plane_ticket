...
<div class="container">
    <div>
        ...
        @endif
        @if(session()->has('login'))
        <button class="col-md-2 btn" style="background: wheat; display: none" id="revenue_by_user">Doanh thu từ {{auth()->user()->name}}: {{number_format($revenue_by_user->total)}} - vnd</button>
        <button class="col-md-2 btn" style="background: wheat; display: none" id="revenue"> Tổng doanh thu: {{number_format($revenue->total)}} - vnd</button>
        @endif
    </div>
</div>
<ul>
